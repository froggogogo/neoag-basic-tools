#!/usr/bin/env bash
# Apply chrom-split fread Sequenza fit (sunbinbin gold) onto a neo tree copy.
set -euo pipefail
...
echo "[ok] applied chrom-split fread Sequenza fit patch -> $FIT_R"
echo "     ensure neoag-sequenza has r-data.table; bam2seqz uses tools/sequenza/bam2seqz_nulsafe.py + samtools 1.9"
