...
本目录是**独立安装模块**。装完后再用兄弟目录 [../neoag-basic-tools-run](../neoag-basic-tools-run/README.md) 跑病例（探查机器 → 基础工具 → 生产报告）。
...
| BigMHC 缺 `src/predict.py` / 不完整 liup 树 | `site.env` 按 sentinel 选完整拷贝；`ensure_bigmhc_predict_py` |
...
- 安装完成后运行：[../neoag-basic-tools-run/README.md](../neoag-basic-tools-run/README.md)
...
`site.env.sh` 在 **source 时**发现本机 conda。缺 env 时在该机跑 `bash scripts/ensure_host_runtime.sh`（可从金标准机 `pack_gold_envs.sh` 灌包）。

耗时说明：从零 copy refs 和第一次建 conda env 可能要数小时；共享 refs 已在盘上时，新机主要耗在补本机 env。
