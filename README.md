...
可移植的新抗原**基础工具链**安装 Skill（A1：单机全量）。版本 **1.1.0**。
...
| 依赖散落多机 | 统一落到 `zzbnew` 的 `neoag_basic_deps` |
...
等价于：copy 资产进 deps + 在 deps 内装/用 Miniforge + 建基础 conda env + 跑 `install_*.sh` + verify。
...
```bash
source /mnt/zzbnew/peixunban/gl/neoag_basic_deps/configs/site.env.sh
```
...
- R 冒烟：`BSgenome`、`BSgenome.Hsapiens.UCSC.hg38`、`sequenza`
